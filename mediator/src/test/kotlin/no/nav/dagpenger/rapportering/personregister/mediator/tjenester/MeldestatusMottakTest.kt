...
class MeldestatusMottakTest {
    private val arenaPersonId = 5268057L
    private val meldestatusId = 95L
    private val hendelseId = 96L
    ...
    fun `onPacket behandler melding og inkrementerer mottakmetrikk`() {
        ...
        verify(exactly = 1) {
            meldestatusMediator.behandle(
                match { hendelse ->
                    hendelse.personId == arenaPersonId &&
                        hendelse.meldestatusId == meldestatusId &&
                        hendelse.hendelseId == hendelseId
                },
            )
        }
        verify(exactly = 1) {
            meldingerRepository.lagreInnkommendeMelding(
                ...
                match { melding ->
                    with(ObjectMapper().readTree(melding)) {
                        this["@event_name"].asString() == "meldestatus" &&
                            this["arenaPersonId"].asLong() == arenaPersonId &&
                            this["meldestatusId"].asLong() == meldestatusId &&
                            this["hendelseId"].asLong() == hendelseId
                    }
                },
            )
        }
