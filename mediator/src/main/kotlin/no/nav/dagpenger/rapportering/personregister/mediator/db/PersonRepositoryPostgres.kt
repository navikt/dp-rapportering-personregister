...
class PersonRepositoryPostgres(
                ...
                .use { session ->
                    session.transaction { tx ->
                        tx.run(
                            queryOf(
                                """
                INSERT INTO fremtidig_hendelse (korrelasjons_id, ident, dato, start_dato, slutt_dato, kilde,referanse_id, type, extra) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?::jsonb)
                ON CONFLICT (referanse_id) 
                DO UPDATE SET 
                    korrelasjons_id = EXCLUDED.korrelasjons_id,
    ...
    private fun tilHendelse(
        ...
        val korrelasjonsId = row.stringOrNull("korrelasjons_id")?.let { UUIDv7.fromString(it) }
